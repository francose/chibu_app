...
resource "aws_ecr_repository" "ecr_chibu_repo" {
  name                 = "chibu-repo"
...
locals {
  container_definitions = [
    {
      name      = "chibu-task_first"
      image     = "587425951590.dkr.ecr.us-east-1.amazonaws.com/chibu-repo:latest"
      essential = true
      memory    = 512
      cpu       = 256
      portMappings = [
...
resource "aws_ecs_service" "app_service" {
  ...
  task_definition = aws_ecs_task_definition.chibu-task.arn
  launch_type     = "FARGATE"
  desired_count   = 3

  load_balancer {
    target_group_arn = aws_lb_target_group.target_group.arn
    container_name   = aws_ecs_task_definition.chibu-task.family
    container_port   = 3000
  }
